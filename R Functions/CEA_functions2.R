  ...
  if (coord.flip == T){
    ggplot(CEmat.df, aes(Effectiveness, Cost)) +
      ...
      geom_point(data = CEmat.df[frontier,], 
                 aes(Effectiveness, Cost, shape = Strategy, color = Strategy), size = 4, stroke = 1) +
      geom_line(data = CEmat.df[frontier,], aes(Effectiveness, Cost), size = 1) +
      scale_shape_manual(values = 19:(n.strategies-1)) +
